<?php
	...
	if (!$seiid) {
		...
		if ($seid && $sdt) {
			...
			if ($seiid) {
				$jump_uri = new UriBuilder('showbuilder.php?seiid='.$seiid);
				$jump_uri->redirect();
			} else {
				exit();
			}
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			refreshScratchpadAndSchedule(function() {
				// Init TinyMCE fields
				initializeKGNUTinyMCEForSelector($('.tinymce'));
	...
	<div>
		<table style="width: 100%; white-space: nowrap">
			<tr>
				...
				<td style="width: 25%; text-align: right"><h5><?php echo date('l, F jS, Y', $scheduledShowInstance->StartDateTime) ?> from <?php echo date('g:i a', $scheduledShowInstance->StartDateTime) ?> to <?php echo date('g:i a', $scheduledShowInstance->StartDateTime + $scheduledShowInstance->Duration * 60) ?></h5></td>
			<tr>
		</table>
			...
			<div id="scratchpad">
				...
				<script type="text/javascript">
					...
					function renderEventInstanceListElement(eventInstance) {
						...
						var eventDetailsList = $('<ul class="eventDetails ui-corner-all"></ul>').click(function() { return false }).hide();
					...
					<div id="Track_tab">
						<script type="text/javascript">
							$(function() {
								$('#Track_edit_AlbumCompilation').change(function(event) {
									if ($('#Track_edit_AlbumCompilation').attr('checked')) {
								...
								$('#Track_edit_AlbumArtist').autocomplete('ajax/autocomplete/artists.php', {
									minChars: 2,
									cacheLength: 0,
									max: 0,
									mustMatch: false,
									...
									Attributes: {
										...
										Album: {
											...
											Attributes: {
												...
												Label: $('#Track_edit_AlbumLabel').val(),
												Local: $('#Track_edit_AlbumLocal').attr('checked'),
												Compilation: $('#Track_edit_AlbumCompilation').attr('checked'),
						...
						<div id="Track_edit">
							<div id="Track_edit_Track">
								...
								<div class="field"><label for="Track_edit_TrackDuration">Duration</label><div class="input"><input type="text" id="Track_edit_TrackDuration"></div></div>
							</div>
