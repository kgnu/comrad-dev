<?php
	...
	if ($uri->hasKey('q')) {
		...
		array_push($mappedCriteria, array('Artist', 'LIKE', "%$q%"));
		
		// Get the results
		$albums = DB::getInstance('MySql')->find(new DBCriteria('Album', $mappedCriteria), $count, array('limit' => $limit, 'groupcolumn' => 'Artist', 'sortcolumn' => 'Artist'));
		
		// Format the results
		$results = array();
		foreach($albums as $album) {
			array_push($results, $album->Artist);
		}
